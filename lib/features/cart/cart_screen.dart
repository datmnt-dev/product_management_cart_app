  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Consumer<CartController>(builder: (context, cart, _) {
            return IconButton(
              tooltip: 'Xóa toàn bộ giỏ hàng',
              onPressed: cart.isEmpty ? null : () => _confirmClear(context, cart),
              icon: const Icon(Icons.delete_sweep_outlined, color: Colors.red),
            );
          }),
        ],
      ),
      body: Consumer<CartController>(builder: (context, cart, _) {
        if (cart.isEmpty) {
          return EmptyState(
            icon: Icons.shopping_basket_outlined,
            title: 'Giỏ hàng của bạn đang trống',
            message: 'Hãy khám phá cửa hàng và chọn những món đồ bạn yêu thích nhé.',
      ...
      bottomNavigationBar: Consumer<CartController>(builder: (context, cart, _) {
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontWeight: FontWeight.w900, color: cs.primary, fontSize: 20),
                  ),
                ],
              ),
    ...
    return Card(
      child: Padding(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ProductImage(imageUrl: product.imageUrl, width: 84, height: 84, borderRadius: 12),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: tt.bodySmall?.copyWith(color: cs.onSurfaceVariant, fontWeight: FontWeight.w700),
                  ),
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        _StepperBtn(
                          icon: Icons.remove,
                          onPressed: () => context.read<CartController>().decrement(product.id),
                        ),
                        Container(
                          width: 38,
                          height: 32,
                          ...
                          decoration: BoxDecoration(
                            border: Border.symmetric(
                              vertical: BorderSide(color: cs.outlineVariant.withValues(alpha: .5)),
                            ),
                          ),
                          child: Text(
                            '${item.quantity}',
                        ...
                        _StepperBtn(
                          icon: Icons.add,
                          onPressed: () => context.read<CartController>().increment(product.id),
                        ),
                      ],
                    ),
                  ),
            ...
            Column(
              ...
              children: [
                Text(
                  ...
                  style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w900, color: cs.primary),
                ),
                const SizedBox(height: 14),
                IconButton(
                  tooltip: 'Gỡ khỏi giỏ hàng',
                  onPressed: () => context.read<CartController>().remove(product.id),
...
Future<void> _checkout(BuildContext context) async {
  ...
  final confirmed = await showDialog<bool>(
    ...
    builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Xác nhận đặt')),
      ],
    ),
  );
  ...
  final order = await context.read<OrderController>().checkout(user: user, items: items);
  cart.clear();
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text('Đơn hàng #${order.id.split('-').last} đã được tạo thành công!'),
