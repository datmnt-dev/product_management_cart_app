...
| Seller | `seller@store.local` | `123456` |
...
| **Seller** | Cửa hàng · Giỏ · Đơn | Quản lý sản phẩm, tồn kho và fulfillment shop mình |
...
- Checkout backend: callable Cloud Function tự tính giá live, áp coupon, trừ kho, tạo đơn và tách fulfillment theo seller.
...
Backend chính: **Firebase Auth + Cloud Firestore + Cloud Functions**. Legacy Hive `LocalDatabase` đã gỡ.

### Deploy checkout backend

Phase 8 chuyển tạo đơn sang Cloud Function `placeOrder`, nên app cần deploy cả
Functions và Firestore rules. Cloud Functions thường cần bật **Blaze plan** cho
Firebase project.

```powershell
Push-Location functions; npm install; Pop-Location
firebase deploy --only functions,firestore:rules --project product-management-cart-app
```

Sau khi deploy, client không còn được phép tự tạo `orders` hoặc
`sellerFulfillments`; các document này chỉ được tạo bởi backend trusted.
...
Seed: users/products/coupons/orders/seller fulfillments/seedRuns document (đủ edge case demo).
...
- Checkout dùng Cloud Function transaction; cancel restock vẫn dùng transaction và `stockRestored` chống double restock.
- Client rules khóa tạo `orders`/`sellerFulfillments`; seller chỉ sửa sản phẩm thuộc shop mình.
